...
for v in db.VIDEOS.find({}).sort('retrieved.-1', 1):
    ...
    if restartId == -999 or not skip:
        ...
        if re.search(r'Private video|ResourceNotFoundException|User authentication required',
                     d.text) or d.status_code == 404:
            print(v.keys())
            ...
            if not 'missing' in v.keys():
                ...
                db.VIDEOS.update({u'videoId': v[u'videoId']}, {'$set': {u'missingReason': [d.text, d.status_code]}},
                                 upsert=False)
